import { CHARACTERS, PLANETS, TAGS, setTags } from './state.js';
import { dbPut } from './db.js';
import { newUuid } from './db.js';
import { esc, showToast, showPrompt } from './utils.js';

// Gallery upload modal tag state (shared with gallery.js via import)
export const mState = {
  file: null, blob: null, editUuid: null,
...
export function populateLoreCustomTags() {
  ...
  container.innerHTML =
    customTags.map(t =>
      `<button class="modal-tag-opt" data-name="${esc(t.name)}">${esc(t.name)}</button>`
    ).join('')
...
export async function addCustomTagInline(context) {
  showPrompt('New Tag', 'Tag name (will be available everywhere)', 'e.g. Mystery', '', async name => {
    ...
    if (context === 'upload-theme' || context === 'upload-custom') populateUploadTags();
    showToast('Tag added');
  });
}

export function initTags() {
  // Delegate clicks for tag options and add-tag buttons in the upload modal
  document.getElementById('uploadModal').addEventListener('click', e => {
    ...
    if (addBtn && addBtn.closest('#uploadModal')) {
      addCustomTagInline(addBtn.dataset.context || 'upload-custom');
    }
  });

  // Lore modal tag clicks
